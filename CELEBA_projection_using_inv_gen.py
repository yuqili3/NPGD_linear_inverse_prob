...
parser = argparse.ArgumentParser(description='NPGD for celebA')
parser.add_argument('--datadir', default='./data/celebA', type=str, help='directory of mnist dataset')
parser.add_argument('--modeldir', default='./trained_model', type=str, help='directory of model training')
parser.add_argument('--resultdir', default='./result/celebA', type=str, help='directory of recovered image results')
...
parser.add_argument('--lr', default=4, type=float, help='Learning rate')   
# Default was 0.5 # need lr = 4 for SR  # need lr = 1 or 2 for inpainting
...
def save_output(output_images, k, iterations, orig_paths, lr):
    ...
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    output_images = output_images.detach().cpu().numpy().squeeze()
...
def save_intermediate_cs(groundtruth, output_images, e,k,m):
    ...
    fig = plt.figure()
    groundtruth = groundtruth.detach().cpu().numpy().squeeze()
    output_images = output_images.detach().cpu().numpy().squeeze()
    ...
    for i in range(B):
        ...
        combined = np.concatenate((t,o),axis=0)
        plt.imshow(combined)
...
def save_intermediate_super(y, groundtruth, output_images, e, k,factor=sr_ratio):
    ...
    y = y.reshape(-1, 3, int(64/factor), int(64/factor)).detach().cpu().numpy()
    ...
    for i in range(B):
        ...
        combined = np.concatenate((p, t, o), axis=0)
        plt.imshow(combined)
        plt.xticks([])
        plt.yticks([])
    plt.savefig(path)
...
def save_intermediate_inpaint(y, groundtruth, output_images, e, k,mask_size=mask_size):
    ...
    groundtruth = groundtruth.detach().cpu().numpy()
    output_images = output_images.detach().cpu().numpy()
    y = y.reshape(-1, 3, 64, 64).detach().cpu().numpy()
    B = groundtruth.shape[0]
    for i in range(B):
        plt.subplot(1, batch_size, i + 1)
        p, t, o = (y[i].transpose(1, 2, 0) + 1) / 2, (groundtruth[i].transpose(1, 2, 0) + 1) / 2, (output_images[i].transpose(1, 2, 0) + 1) / 2
...
def get_scatter_inpaint_A(mask_size, num_block):
    np.random.seed(0)
    A=np.ones((3,64,64))
    x,y=np.random.choice(np.arange(64),size=num_block),np.random.choice(np.arange(64),size=num_block)
    for i in range(num_block):
        A[:,x[i]:x[i]+mask_size, y[i]:y[i]+mask_size]=0
    A = np.diag(A.reshape(-1))
    return A


gen_path = '%s/CELEBA_DCGAN_results_%d/generator_param.pkl'%(model_dir, k)
G = models.DCGAN_generator(k=k, d=128)
G.load_state_dict(torch.load(gen_path))
G.cuda()

invG_path = '%s/CELEBA_DCGAN_results_%d/invGen_param_mtl_loss_noisyGANoutput_sigma%.2f_lamda%.2f.pkl'%(model_dir,k,invGsigma, invGlamda);
...
if task == 'cs':
    ...
    if use_designed_mat:
        ...
        A = torch.load("%s/CELEBA_DCGAN_results_%d/designed_matrix_A_m%d.pkl"%(model_dir,k,m));
    else:
        A = np.random.normal(loc=0,scale=1/np.sqrt(m), size=(m, 3, 64, 64))
...
# Perform A@x in torch, except now x: Bx3x64x64, A: mx3x64x64, return shape Bxm
def Aat(x):
    y = (A*x.unsqueeze(1)).sum(dim=(2,3,4))
    return y
# Perform A.T@y in torch, except now y: Bxm, A: mx3x64x64, return shape Bx3x64x64
def ATat(y):
...
if save_intermediate:
    ...
    imageio.mimsave('%s/celebA_%s_example.gif'%(figure_dir, task_name), images, fps=5)
